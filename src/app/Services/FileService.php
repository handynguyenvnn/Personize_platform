...
class FileService
{
    ...
    {
        return Storage::disk($this->disk)->url('/') . $this->getFilePath($filename);
    }

    public function getFullFilePathUsingFilePath($filePath)
    {
    ...
    public function uploadBase64File($filename, $file)
    {
        $filePath = $this->getFilePath($filename);
        try {
